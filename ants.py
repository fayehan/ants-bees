    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.name=='Queen':
                if insect.first_queen==False:
                    self.ant=None
            elif insect.container==False:
                self.ant = None
        ...
        insect.place_temp=insect.place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR CODE HERE ***"
        entrance_temp=self.place.entrance
        step=0
        while entrance_temp.bees==[] or step<self.min_range:
            entrance_temp=entrance_temp.entrance
            step+=1

        bee_list=entrance_temp.bees
        if self.place.bees!=[]:
            return random_or_none(self.place.bees)
        else:
            if entrance_temp==hive or step>self.max_range:
                return None
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            bee_list=list(self.place_temp.bees)
            for bee in bee_list:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.first_queen==True:
            ...
            while temp_place.entrance is not None:
                if temp_place.ant is not None and temp_place.ant not in self.doubled_ant:
                    temp_place.ant.damage*=2
                    if temp_place.ant.container:
                        if temp_place.ant.ant.name!='Queen':
...
def apply_effect(effect, bee, duration):
    ...
    i=0
    a=bee.action
    effect_bee_action=effect(bee.action)
    def apply(colony):
        nonlocal i
        if i<duration:
            effect_bee_action(colony)
            i+=1
        else:
            a(colony)
